  ...
  actions: {
    ...
    getProvinces(): string[] {
      ...
      if (
        ...
        this.provinceIndex >= this.provinceList.length
      )
      ...
      let resArray = [];
      while (
        resArray.length < NUM_STEP &&
        newIndex < this.provinceList.length
      ) {
        let province = this.provinceList[newIndex];
        if (!this.provinceChosed.includes(province)) resArray.push(province);
        newIndex++;
      }
    ...
    getCities(): string[] {
      ...
      while (resArray.length < NUM_STEP && newIndex < this.cityList.length) {
        ...
        if (!this.provinceChosed.includes(city)) resArray.push(city);
        newIndex++;
      }
      // info update information
